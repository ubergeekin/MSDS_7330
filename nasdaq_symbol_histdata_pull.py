...
symbol = []
with open('D:/TradingAnalysis/nasdaq/nasdaq_symbols_clean.csv') as f:
    for line in f:
...
path_out = ('D:/TradingAnalysis/nasdaq/')
...
while i<len(symbol):
    try:
        ...
        if i == 0:
            df.to_csv(path_out+'nasdaq_prices_volumes.csv')
            print (i, symbol[i],'has data stored to csv file')
        else:
            df.to_csv(path_out+'nasdaq_prices_volumes.csv',mode = 'a',header=False)
            print (i, symbol[i],'has data stored to csv file')
    continue
    i=i+1
